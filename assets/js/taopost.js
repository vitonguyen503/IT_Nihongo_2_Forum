document.addEventListener('DOMContentLoaded', function () {
    ...
    // Thêm sự kiện click cho từng nút "Trích dẫn"
    openPopupButtons.forEach(button => {
        button.addEventListener('click', function() {
            modal1.style.display = "flex";  // Hiển thị modal khi nút được nhấn
        });
    });

    // Đóng modal khi bấm vào nút đóng
    closeBtn1.addEventListener('click', function() {
        modal1.style.display = "none";  // Ẩn modal khi bấm vào nút đóng
    });

    // Đóng modal khi bấm ra ngoài modal
    window.addEventListener('click', function(event) {
        if (event.target === modal1) {
            modal1.style.display = "none";  // Ẩn modal khi bấm ra ngoài
        }
    });
